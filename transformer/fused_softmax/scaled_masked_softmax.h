 ...
 * original data "input" is a 4d tensor with dimensions [batches, attn_heads, seq_len, seq_len]
 * param: micro_batch_size =  batch_count = batches * attn_heads * query_seq_len;
 * param: element_count: key_seq_len = input.size(3). Last dim of input.
 * param: pad_batches = batches.
*/
    ...
    int pad_batches)
{
    ...
    // The log2_elements: log2_ceil(key_seq_len). Then next_power_of_two is key_seq_len;
    constexpr int next_power_of_two = 1 << log2_elements;
    // C10_WARP_SIZE: #define C10_WARP_SIZE warpSize #define C10_WARP_SIZE 32
    constexpr int WARP_SIZE = (next_power_of_two < C10_WARP_SIZE) ? next_power_of_two : C10_WARP_SIZE;
    constexpr int WARP_ITERATIONS = next_power_of_two / WARP_SIZE;
    // less key_seq_len lefts more register for warp_batch.
    constexpr int WARP_BATCH = (next_power_of_two <= 128) ? 2 : 1;\
    // LDG: load. STG: store. While data is more then single thread carrys more data.
    ...
    // blocks(query_seq_len/batches_per_block, attn_heads, batches)
    ...
    int element_count)
{
    ...
    // acc might be change. using two more temp vars to copy data:
