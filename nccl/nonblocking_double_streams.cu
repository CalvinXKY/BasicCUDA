...
struct ThreadArgs {
    ...
    ncclUniqueId* id;
    int uuid=-1;
    ThreadArgs(int gpu_id,int global_size, ncclUniqueId* id):gpu_id(gpu_id), id(id), \
    global_size(global_size){};
    ThreadArgs(int gpu_id,int global_size, ncclUniqueId* id, int uuid):gpu_id(gpu_id), id(id), \
    global_size(global_size), uuid(uuid){};
};
...
bool cmpID(ncclUniqueId* id1, ncclUniqueId* id2) {
    if (memcmp(id1->internal, id2->internal, sizeof(id2->internal))==0)
    {
        ...
        for (int i = 0; i< 128; i++) {
            ...
            if (id1_ch != id2_ch) printf("Id diff internal idx_%d: id1:%c id2:%c\n", i, id1_ch, id2_ch);
        }
        return true;
    }
}
...
public:
    ncclUniqueId* id_ref;
    CommExec(int gpu_nums):gpu_nums(gpu_nums) {}
...
void* allReduceOps(void* args) {
    ...
    if(if_debug) printTimestamp(gpu_id, threadArgs->uuid, "start");
    ...
    if(if_debug) printTimestamp(gpu_id, threadArgs->uuid, "first iter end");
    ...
    if(if_debug) printTimestamp(gpu_id, threadArgs->uuid, "end");
...
int main(int argc, char* argv[]) {
    ...
    if(if_debug) cmpID(commexec1.get(), commexec2.get());
    commexec2.wait();
    commexec1.wait();
